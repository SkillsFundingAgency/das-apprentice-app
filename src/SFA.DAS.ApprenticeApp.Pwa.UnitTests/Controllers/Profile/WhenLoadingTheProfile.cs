...
namespace SFA.DAS.ApprenticeApp.Pwa.UnitTests.Controllers.Profile
{
    public class WhenLoadingTheProfile
    {
            ...
            using (new AssertionScope())
            {
                logger.Verify(x => x.Log(LogLevel.Information,
                    ...
                    It.Is<It.IsAnyType>((object v, Type _) =>
                            v.ToString().Contains($"Profile attempted load but user not accepted terms 'apprenticeDetails' {apprenticeId}")),
                    It.IsAny<Exception>(),
            ...
            var claimsPrincipal = new ClaimsPrincipal(new[] {new ClaimsIdentity(new[]
        {
            ...
            using (new AssertionScope())
            {
                logger.Verify(x => x.Log(LogLevel.Information,
                    It.IsAny<EventId>(),
                    It.Is<It.IsAnyType>((object v, Type _) =>
                            v.ToString().Contains($"Profile Index 'apprenticeDetails' is null for {apprenticeId}")),
                    It.IsAny<Exception>(),
                    (Func<It.IsAnyType, Exception, string>)It.IsAny<object>()));
        ...
        [Test, MoqAutoData]
        public async Task Then_The_Home_Page_Is_Displayed_For_Unauthorised_Apprentice([Greedy] ProfileController controller)
        {
            var httpContext = new DefaultHttpContext();
